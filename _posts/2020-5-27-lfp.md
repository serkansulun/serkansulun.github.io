---
layout: post
title: Learned frame prediction
style: wide
---

We present the qualitative results for our learned video prediction model. In particular, we compare the performance of two models, one trained with L2 loss (L2) and the other with adversarial loss and L2 loss combined (GAN). We show that GAN creates sharper predictions at the cost of higher error. All results belong to the 9th frame of each video. The slideshow provides an easy way to compare images, enabling navigation using the arrow keys of the keyboard. Below the slideshow, ground-truth and prediction images are available at their original size.

<br/><br/>

<center>
<iframe src="https://docs.google.com/presentation/d/e/2PACX-1vSbM364gi0ZpIS-Ms_1Ty4p5EVolJJ_XpPIPCRkdSLRnaxeZ6FU7iEldHzBFRkNkqvSNco2cTIXl4RB/embed?start=false&loop=false&delayms=60000" frameborder="0" width="1319" height="770" allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"></iframe>
</center>

<br/><br/>

# City - Frame 9

## Ground-truth

![](../images/lfp/gt_city_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_city_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_city_009.png)

<br/><br/>

# Coastguard - Frame 9

## Ground-truth

![](../images/lfp/gt_coastguard_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_coastguard_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_coastguard_009.png)

<br/><br/>

# Container - Frame 9

## Ground-truth

![](../images/lfp/gt_container_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_container_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_container_009.png)

<br/><br/>

# Football - Frame 9

## Ground-truth

![](../images/lfp/gt_football_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_football_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_football_009.png)

<br/><br/>

# Foreman - Frame 9

## Ground-truth

![](../images/lfp/gt_foreman_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_foreman_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_foreman_009.png)

<br/><br/>

# Garden - Frame 9

## Ground-truth

![](../images/lfp/gt_garden_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_garden_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_garden_009.png)

<br/><br/>

# Hall monitor - Frame 9

## Ground-truth

![](../images/lfp/gt_hall_monitor_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_hall_monitor_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_hall_monitor_009.png)

<br/><br/>

# Harbour - Frame 9

## Ground-truth

![](../images/lfp/gt_harbour_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_harbour_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_harbour_009.png)

<br/><br/>

# Mobile - Frame 9

## Ground-truth

![](../images/lfp/gt_mobile_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_mobile_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_mobile_009.png)

<br/><br/>

# Tennis - Frame 9

## Ground-truth

![](../images/lfp/gt_tennis_009.png)

<br/><br/>

## Prediction of L2 model

![](../images/lfp/pred_L2_tennis_009.png)

<br/><br/>

## Prediction of GAN model

![](../images/lfp/pred_GAN_tennis_009.png)

<br/><br/>
